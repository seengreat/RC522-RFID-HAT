...
unsigned char UART3_ReadWriteByte(u8 TxData)
{		
...
{
	return UART3_ReadWriteByte((Address&0x3F) | 0x80);
}
...
{
	unsigned char ch = UART3_ReadWriteByte (Address& 0x3F);  // 写入寄存器地址(最低6位为实际地址，次高为保留为0，最高位为1)
	if(ch != Address) 
	{
		printf("not equal!!\r\n");
	}
	UART3_ReadWriteByte (value);
}
