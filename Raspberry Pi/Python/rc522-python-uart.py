...
# This demo is used for uart interface
...
class Rc522_api():
    def __init__(self):
        ...
        self.status = 0
        self.block_num = 0x08
        self.com = serial.Serial("/dev/ttyS0", 9600)  # You need to map serial0 to ttyAMA0 first,and then
                                                        # you can use the uart of the expansion port normally
        print('serial init')
    ...
    def uart_readwrite_byte(self, txdata):
        rev = b''
        self.com.flushInput()
        senddata = bytes([txdata])
        self.com.write(senddata)
        timeout = 3
        while timeout > 0:
            time.sleep(0.003)  # when the bandrate is 9600:time.sleep(0.003)
                               # when the bandrate is 115200:time.sleep(0.001)
            cnt = self.com.inWaiting()
            if cnt == 1:
                rev = self.com.read(1)
                break
            timeout -= 1
    ...
    def write_rawrc (self, ucaddress, ucvalue):
        ...
        ch_byte = self.uart_readwrite_byte(ucaddress & 0x3F)  #register addr(the lowest 6 bits are the actual address，MSB:1)
        ch_int = int.from_bytes(ch_byte, 'little')
        if ch_int != ucaddress:
            print("not equal!!")
        self.uart_readwrite_byte(ucvalue)

    def read_rawrc(self, ucaddress):
        """read current value from the register"""
        res = self.uart_readwrite_byte((ucaddress & 0x3F) | 0x80)
        if res != b' ':
    ...
    def pcd_reset(self):
        ...
        time.sleep(0.001)
        # self.PcdSet_Uart_band(115200)  # available on the RC522 board power off and restart
...
if __name__ == '__main__':
    print(" ========================================================\n");
    print(" |	SW1:   RX   -> ON			SW2:	A1	 -> -      |\n");
    print(" |          TX   -> ON					A0	 -> -      |\n");
    print(" |          SDA  -> OFF					ADR5 -> 0      |\n");
    print(" |          SCL  -> OFF					ADR4 -> 0      |\n");
    print(" |          NSS  -> OFF					ADR3 -> 0      |\n");
    print(" |          MOSI -> OFF					ADR2 -> 0      |\n");
    print(" |          MISO -> OFF					ADR1 -> 0      |\n");
    print(" |          SCK  -> OFF					ADR0 -> 0      |\n");
    print(" ========================================================\n");

    rc522 = Rc522_api()
    wiringpi.digitalWrite(29,1)  # turn on red led
    wiringpi.softPwmCreate(24, 0, 8)  # turn on buzzer
    
    print("RC522_UART_TEST...")
    rc522.init()

    print(" start write and read card...")

    while True:
        if rc522.write(rc522.block_num, "0123456789112233"): # if necessary,the content can be modified to other
            print('write card success')
            break
    while True:
        if rc522.read(rc522.block_num):  # read the content written to the card in the previous step
